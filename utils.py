...
def get_cifar10_loaders(datapath, batch_size, cutout, n_holes=1, length=16):
...
def get_cifar100_loaders(datapath, batch_size, cutout, n_holes=1, length=16):
    normalize = transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],
                                    std=[0.2023, 0.1994, 0.2010])
    ...
    trainset = torchvision.datasets.CIFAR100(root=datapath, train=True, download=False, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)

    testset = torchvision.datasets.CIFAR100(root=datapath, train=False, download=False, transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=2)

    return trainloader, testloader

def get_imagenet_loaders(datapath, batch_size):
    ...
    trainset = torchvision.datasets.ImageNet(root=datapath, split='train', download=False, transform=transform_train)
    testset = torchvision.datasets.ImageNet(root=datapath, split='val', download=False, transform=transform_test)
...
def validate(model, epoch, valloader, criterion):
    global acc_history
...
def number_of_zeros(model):
    ...
    print("Length || FullNet : {}, Mask : {} ||".format(length, mask_length))
    print("Remaining Weight Percent || FullNet : {:.2f}%, Mask : {:.2f}% ||".format((nonzeros / length) * 100, (mask_nonzeros / length) * 100))
    
    return (mask_nonzeros / length) * 100
...
def save_checkpoint(state, expname):
    ...
    torch.save(state, filepath+expname+'.pth.tar')
